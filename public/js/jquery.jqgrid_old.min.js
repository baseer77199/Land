	...
	return this.each( function() {
		...
		constructTr = function(id, hide, classes, rd, cur ) {
			...
			if(!$.isEmptyObject( rowAttrObj )) {
				...
				if(typeof(cur.active) != "undefined"){
			if(cur.active=='No')
			{
				style="background-color: #ff000014";
		...
		populate = function (npage) {
			if(!ts.grid.hDiv.loading) {
				...
				var export_data=$.isFunction(ts.p.serializeGridData)? ts.p.serializeGridData.call(ts,ts.p.postData) : ts.p.postData;
				
				
				// $.cookie(ts.id+'_export_url',JSON.stringify(ts.p.url));
				// $.cookie(ts.id+'_export_data',JSON.stringify(export_data));
	...
	exportToExcel : function ( o ) {
	   ...
	   console.log(fulldata);
	    var data=new Array();
			...
			var url='';//$.trim($.cookie(id.substr(1)+'_export_url')).slice(1,-1);
			if(url!='')	
			{
				
			var url_data=JSON.parse($.cookie(id.substr(1)+'_export_data'));
			url_data.rows=0;
			url_data.download=1;
...
$.get(url, url_data, function(downloaddata)
                      { 
					
					
					 body_data=downloaddata;	
   
exceldownload(fulldata,body_data,o);
            ...
            $(tableid +'> tbody  > tr').each(function(index) {
             var array ={}; 
              if(index!=0)
              {
                $(this).find('td').each(function(index1,val) {
                 var id=$(this).attr('aria-describedby').slice(count);
...
pdfdownload(fulldata,body_data,o);
...
function exceldownload(fulldata,body_data,o)
{
...
function pdfdownload(fulldata,body_data,o)
	{
		...
		return fulldata.each(function() {
			var $t = this, rows = [], j, cm = $t.p.colModel, ien, obj = {}, key,
			data = body_data, def = [], i=0, map=[], test=[], widths = [],  align={};
