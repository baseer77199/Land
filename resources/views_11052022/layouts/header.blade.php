...
$allowed_host=array('172.31.61.248:156','192.168.1.13','localhost','localhost:156','172.31.61.248','103.74.215.40','ticketingsolutions.lntecc.com','ticketingsolutions.lntecc.com:156');
                ...
                <div class="notification_home">
                  <?php if(count($count_no) > 0 ){ foreach ($count_no as $key => $value) { ?>
                      ...
                      <div  <?php if($value->noty=="read") { ?> class="alert alert-success" <?php } else {?> class="alert alert-info" <?php  } ?>>
                              ...
                              <form id="noti-form{{$key}}" action="{{URL::to($value->reference_url.'?btnval='.$value->btn_val.'&ref_id='.$value->notification_id)}}" method="POST" style="display: none;">
                                {{ csrf_field() }}
                                <input type='hidden' name='notif_id' value='{{$value->reference_source_id}}' id='notif_id' class='notif_id'>   
                                <input type='hidden' name='btnval' value='{{$value->btn_val}}' id='btnval' class='btnval'>   
                              </form>  
                      <a class="noti_lines" data-val="{{$key}}" <?php if($value->noty=="read") { ?> style="color: green;" <?php } else {?> style="color: blue;" <?php  } ?> >
