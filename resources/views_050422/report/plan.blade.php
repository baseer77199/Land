...
<body>
 ...
 <tr>
      <td  ><img class="img" src='../public/images/logo.jpg' width="200" height="50">
      ...
      <th width="20%" class="txtcenter">PREVENTIVE MAINTENANCE SCHEDULE </th>
  </tr>  
    </table>
    <table width="100%" border="1" class="sbox-content" id="table2">
        
<thead>
 
<tr>
<th rowspan="3" class="sno" >SNO</th>
<th rowspan="3" class="machnineno" >Machine Number</th>
<th rowspan="2">{{$year}}</th>

<?php foreach($month1 as $key=>$value){ ?>
<th colspan="{{$weeks[$key]}}" class="thcolor" >{{$value}}</th>
...
<?php foreach($month1 as $key=>$value){ ?>
<th colspan="{{$weeks[$key]}}" >Planned Week</th>
...
<tbody>
    ...
    <tr>
        ...
        <td rowspan="2" class="sno" >{{$sno}}</td>

        <td rowspan="2" class="machnineno" >{{$value->asset_code}}-{{$value->machine_name}}</td>
        <td >Plan</td>
        <?php  foreach($month1 as $key=> $value) { 
$no_of_weeks=$weeks[$key]; 
//$no=$no+$i;	
...
if(isset($data[$machine_id][$key][$i])) {

      if(!empty($data[$machine_id][$key][$i]['date'])) {
echo"
<td class='tooltip1'><img src='../public/images/plan.png'>
  <span class='tooltiptext1'>".$data[$machine_id][$key][$i]['date']."</span>
</td>";
} else { echo "<td ></td>"; }
...
if(isset($data[$machine_id][$key][$i])) {
     //    echo  "<td>".$data[$machine_id][$no]['date']."</td>";
     /* echo  "<td><img src='../public/sximo/images/plan.png'></td>";*/
     if(!empty($data[$machine_id][$key][$i]['done_date'])) {
     echo"
<td >".$data[$machine_id][$key][$i]['done_date']."
</td>";
      } else { echo "<td ></td>"; }
                                    } else {
                                        echo "<td ></td>";
