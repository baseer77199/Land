...
<?php if($pageMethod=="documents") { ?>
<h2 class="heads">NEW CALIBRATION </h2>
<?php } else {?>
<h2 class="heads">RENEWAL CALIBRATION</h2>
<?php }?>
 <form id="create-user" action="{{URL::to('documentcreate')}}" method="POST" style="display: none;">
                                    {{ csrf_field()}}
                                <input type='hidden' name='id' value='0' id='id' class='id'>   
                                    
                                  </form>
                                  <form id="renewal-user" action="{{URL::to('documentrenewal')}}" method="POST" style="display: none;">
                                    {{ csrf_field()}}
...
$("#grid1").jqGrid({
    <?php if($pageMethod=="documents") { ?>
url: "{{URL::to('documentData')}}",
    <?php } else { ?>
        url: "{{URL::to('renewalData')}}",
    <?php } ?>
datatype: "json",
mtype: "GET",
	 colModel: [
        { name: "document_id", label: "id",hidden:true },
	//{ name: "form_no", label: "Form No. or Document Needed"},
	{ name: "department_name", label: "Department"},		
        { name: "machine_name", label: "Machine"},
        { name: "frequency_name", label: "Frequency"},
	{ label: "Renewal Date", name: "renewal_date"},		
	{ label: "Valid From", name: "valid_from"},		
	{ label: "Valid To", name: "valid_to"},		
	//{ label: "Fee", name: "fee"},		
...
showcolumn('grid1');
 ...
 $(document).on('click',".exportpdf",function() {
  ...
  includeFooter: true,
  fileName : "Document Management.pdf",
 ...
 $(document).on('click',".exportexcel",function() {	
	$("#grid1").jqGrid("exportToExcel",{
					includeLabels : true,
    					includeGroupHeader : true,
    					includeFooter: true,
    					fileName : "Document Management.xlsx"
...
var gr = jQuery("#grid1").jqGrid('getGridParam','selrow');
  var cellValue = jQuery("#grid1").jqGrid ('getCell', gr, 'document_id');
   var id=$('.id').val(cellValue);
   if(gr){ 
document.getElementById('renewal-user').submit(); 
...
$(document).on('click','.del',function(e){
        ...
        if(cellValue)
        {
            swal({
               ...
               var red_url     ="{{ url('document') }}";
                 if(e == true)
                 {
                    $.get('documentdelete/'+cellValue, function(data,status)
                    {
                        var data = $.trim(data);
                      //  alert(data);
                        if(data =='0')
...
//Ajith purpose : function  clearsearch in grid  
 ...
 /*end*/
  ...
  var cellValue = $("#grid1").jqGrid ('getCell', gr, 'document_id');
     var id=$('.id').val(cellValue);
        if(gr){     
...
//   if(gr)
//   {
//      var url="{{URL::to('documentview')}}/"+cellValue;
//      window.location.replace(url);
//   }
//   else
//   {
