...
<?php if($pageMethod=="closedrequest"){$datetimepicker="datpicker2";}else{$datetimepicker="datetimpicker1";} ?>
...
<h2 class="heads">  <?php if($pageMethod=="allocateengineer") {?>
                                     ...
                                     <?php } else if($pageMethod=="approverequest"){ ?>
                                      ...
                                      <?php } else if($pageMethod=="closerequest"){?>
                                        Close Request
                                      <?php } else if($pageMethod=="closedrequest"){?>
                                        Update Issue
...
<div class="card-body card-block">
  ...
  <div class="row">
    ...
    <div class="col-md-4">
            ...
            <div class="col-md-8">
              <input type="hidden" name="id" id="id"  class="ticket_id" value="{{ $row->id }}">
...
<?php } ?>
 <?php if($pageMethod=="approverequest" || $pageMethod=="closerequest" || $pageMethod=="closedrequest") { ?>
    ...
    <div class="col-md-4">
  <?php  if($pageMethod=='createissue'){$style = "style=pointer-events:none";}else{$style = "style=pointer-events:none";} ?>
<div class="form-group row" {{$style}}>
  ...
  <div class="input-group date form_date col-md-12 datehide">
      <input class="form-control  issue_date" id="issue_date" name="issue_date" size="16" type="text" value="{{ $row->issue_date }}" required readonly>
 ...
 <?php if($pageMethod=="requestraise" || $pageMethod=="approverequest" || $pageMethod=="closerequest" || $pageMethod=="closedrequest") { 
 ...
 <div class="form-group row requestraise" {{$style}}>
  ...
  <div class="col-md-8">
  <div class="input-group date form_date col-md-12 datehide">
      <input class="form-control start_date {{$datetimepicker}}" id="start_date" name="start_date" size="16" type="text" value="{{ $row->start_date }}" required>
...
<div class="form-group row approverequest" >
  ...
  <div class="col-md-8">
  <div class="input-group date form_date col-md-12 datehide">
      <input class="form-control end_date {{$datetimepicker}}" id="end_date" name="end_date" size="16" type="text" value="{{ $row->end_date }}" required>
...
<!-- raja Code for lines level data-->
           ...
           <div class="col-md-12">
             <a href="javascript:void(0);"  class="add_row additem"  rel=".rcopy"><i class="fa fa-plus"></i> New Item</a>
                ...
                </thead>  <?php //dd(count($productdata)); ?>
                 <tbody class="product_tbl_lines_body">
                   <?php if(count($productdata)>=1){ ?>
                   @foreach($productdata as $key=>$value)
                     <tr>
                        ...
                        <td>
                          <input type="text" name="bulk_qty[]" class="form-control input-sm bulk_qty"  data-value="0" value="{{$value->qty}}">
                        </td>
                        <td>
    ...
    <div class="col-lg-12 col-md-12">
        <div class="form-group text-center">
          ...
          <?php }else if($pageMethod=="closerequest"){?>
           ...
           <?php  }else if($pageMethod=="closedrequest"){?>
          	<button name="submit" type="button" class="btn save saveform" value="UPDATED">Update</button>
...
$(document).ready(function()
{
    ...
    $('.technician').change(function () {
    var technicianList = $(this).val();
    if (technicianList && technicianList.length > 0) {
        var technician = technicianList[0];
        if (technician !== '') {
            $('.engineer').val(technician);
        }
    } else {
...
// alert(page);
  ...
  $('.pagemethod').attr('readonly',true);
    ...
    }else if(page=="closerequest" || page=="closedrequest"){
...
$('.others').hide();
$('.linesdiv').hide();
if(page=='closedrequest'){
  $('.pagemethod').attr('readonly',false);
  $('.bdmachine_div').css('pointer-events','auto');
  $('.pagemethod,.engineer,.technician_div,.requestraise').css('pointer-events','auto');
  $('.closerequest').show();
  $('.closere').css('pointer-events','auto');
  $('.active').show();
  $('.critical_spare,.preventive_action,.corrective_action').attr('required',true);
 ...
 $(document).on('change','#machine_id',function()
   {
      ...
      //alert(issue_id);
       ...
       $.get(url,function(data){
          if(data==1)
          {
            <?php if($pageMethod!="closedrequest"){?>
...
var issueDate = $(".issue_date").data("datetimepicker").getDate() ;
...
if(issueDate>startDate){
  notyMsgs('info',"Start Time Must be greater than Issue Time");
  $(".start_date").val('');
} 
