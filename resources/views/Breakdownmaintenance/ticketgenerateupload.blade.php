...
<h2 class="heads">Ticket Generate Upload</h2>
            ...
            <div class=" col-md-6">
                ...
                <form id="file_up">
                    ...
                    <div class="col-md-offset-2 col-md-10" style="margin-left: 15.666667%;">
                        ...
                        <!--<a href="{{url('/download/ticketgenaratechecklist.csv')}}"  class="lst btn download" download>Download Template</a>-->

                    </div>
                    <!--  </div> -->
                </form>
                <form id="file_up_download" method="POST" action="{{URL::to('download_templeteticketgenarate')}}">
                    {{ csrf_field()}}
            ...
            <div class=" col-md-6">
                ...
                <form id="validate_form" action="">
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <div class="col-md-6">
                            <select id="batch_number" name="batch_number" class="select2 form-control batch_number"
                                required="">
                                {!!$batch_no!!}
                            </select>
                        </div>
...
$(document).ready(function() {
    ...
    $("#grid1").jqGrid({
        url: "getticketgenarateuploaddata",
        datatype: "json",
        mtype: "GET",
        colModel: [{
                name: "ticketgenarate_id",
                label: "id",
                width: 100,
                hidden: true
            },
    ...
    $("#editdata").click(function() {
        ...
        var cellValue = $("#grid1").jqGrid('getCell', gr, 'ticketgenarate_id'); //alert(cellValue);
        ...
        if (cellValue) {
            if (batch_status == "LOADED") {
                ...
                window.location.replace('ticketgenarateuploadedit/' + cellValue);
            }
        } else {
    ...
    $('.verify').click(function() {
        ...
        if (batchname != '') {
            ...
            var url = "{{URL::to('getticketgenaratevalidate')}}";
    ...
    $(".download").click(function() {
        ...
        $.ajax({
            url: "{{URL::to('download_templeteticketgenarate')}}",
            type: "POST",
            data: form_data,
            ...
            xhr: function() {
                ...
                if (xhr.upload) {
                    xhr.upload.addEventListener('progress', function(event) {}, true);
                }
                return xhr;
            }
    ...
    $(document).on('click', '#upload', function() {
        ...
        if (file_upload != '') {
            ...
            var form_data = new FormData(document.getElementById('file_up'));
            $.ajax({
                url: "{{URL::to('ticketgenarateuploadsave')}}",
                type: "POST",
                data: form_data,
                enctype: 'multipart/form-data',
                processData: false, // tell jQuery not to process the data
                ...
                if (data == 1) {
                    ...
                    notyMsgs('success', 'ticketgenarate Checklist Uploaded  Successfully');
                    setTimeout(function() {
                        location.reload();
                    }, 2000);
                } else if (data == 2) {
    ...
    $(document).on('click', '.delete', function(e) {
        ...
        var cellValue = jQuery("#grid1").jqGrid('getCell', gr, 'ticketgenarate_id');
        var batch_status = jQuery("#grid1").jqGrid('getCell', gr, 'batch_status');
        if (cellValue) {
            if (batch_status == "LOADED") {
                ...
                $.post("{{url('ticketgenarateuploaddelete')}}", {
                    id: cellValue
