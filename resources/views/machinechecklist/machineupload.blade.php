...
<div class="card">
    <div class="card-body card-block">
        <div class="row">
            <div class=" col-md-6">
            <form id="edit-form" method="POST" action="{{URL::to('checklistuploadedit')}}" enctype="multipart/form-data" style="display:none;">
                {{ csrf_field()}}
                ...
                <form id="file_up_download"  method="POST" action="{{URL::to('download_templetemachinechecklist')}}">
                    {{ csrf_field()}}
...
<script>
	$(document).ready(function(){
        ...
        $("#grid1").jqGrid({
	        url: "getchecklistuploaddata",
	        datatype: "json",
	        mtype: "GET",
            colModel: [
                { name: "checklist_hrd_id", label: "id", width:100,hidden:true },
        ...
        $("#editdata").click(function()
        {
            ...
            var cellValue = $("#grid1").jqGrid ('getCell', gr, 'checklist_hrd_id');  //alert(cellValue);
		...
		$('.verify').click(function()
        {
		    ...
		    if(batchname!=''){
		        ...
		        var url="{{URL::to('getchecklistvalidate')}}";
		...
		$(".download").click(function(){
            ...
            $.ajax({
                url: "{{URL::to('download_templetemachinechecklist')}}",
                type: "POST",
                data: form_data,
...
;
                if(file_upload != '')
                {
                    ...
                    var form_data = new FormData(document.getElementById('file_up'));
                    $.ajax({
                          url: "{{URL::to('machinechecklistuploadsave')}}",
                          type: "POST",
                          data: form_data,
                          enctype: 'multipart/form-data',
                          processData: false,  // tell jQuery not to process the data
...
/** delete funcation start ***/
            $(document).on('click','.delete',function(e){
                ...
                var cellValue = jQuery("#grid1").jqGrid ('getCell', gr, 'checklist_hrd_id');
                var batch_status = jQuery("#grid1").jqGrid ('getCell', gr, 'batch_status');
                if(cellValue)
                {
                    if(batch_status=="LOADED"){
                        ...
                        $.post("{{url('checklistuploaddelete')}}", {id: cellValue}, function(data,status)
                        {
                            if(data == 1)
                            {
