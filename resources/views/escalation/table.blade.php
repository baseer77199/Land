...
<form id="create-user" action="{{URL::to('escalationcreate')}}" method="POST" style="display: none;">
                                   {{ csrf_field() }}
    ...
    <form id="view-user" action="{{URL::to('escalationview')}}" method="POST" style="display: none;">
                                  {{ csrf_field() }}
                                <input type='hidden' name='id' value='0' id='id' class='id'>   
...
$("#grid1").jqGrid({
url:"{{URL::to('getescalationdata')}}",
mtype:'GET',
datatype:'json',

colModel: [
{ name: "id", label: "S.NO",hidden:true },
{ name: "escalation_hrd_id", label: "id",hidden:true },
{ name: "statuss", label: "Condition" },
...
// 	var edit_id = $("#grid1").jqGrid ('getCell', index, 'escalation_hrd_id');
//       if( index){
...
showcolumn('grid1');
 ...
 $('#viewdata').on('click',function(){
     var index = $("#grid1").jqGrid('getGridParam','selrow');
 	var id = $("#grid1").jqGrid ('getCell', index, 'escalation_hrd_id');
     var id=$('.id').val(id);
        if(index){     
