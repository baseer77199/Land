...
@media print {
    ...
    #reportshow:after {
        visibility: visible;
        content: "FORMAT NO:04.2211.71.03-6 REV:00/16";
    }
}
</style>
        ...
        $(document).on('click', '.search', function() {
            ...
            if (start_date != 0 && end_date != 0) {
                if (start_date <= end_date) {
                    var url = "{{URL::to('breakdownregistersearch')}}/" + start_date + "/" + end_date;
                    $.get(url, function(data) {
                        $('.monthlybreakdownreport').html(data);
                    });
                } else {
                    $('.monthlybreakdownreport').html('');
                    notyMsg("info", "End Date must be greater than or equal to Start Date");
                }
            } else {
                $('.monthlybreakdownreport').html('');
