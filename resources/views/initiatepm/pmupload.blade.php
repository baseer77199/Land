...
<h2 class="heads">PM Machine Upload</h2>
<div class="card">
    <div class="card-body card-block">
        <div class="row">
            <div class=" col-md-6">
                <form id="edit-form" method="POST" action="{{URL::to('checklistuploadedit')}}"
                ...
                <form id="file_up">
                    ...
                    <div class="col-md-offset-2 col-md-10" style="margin-left: 15.666667%;">
                        ...
                        <!--<a href="{{url('/download/pm.csv')}}"  class="lst btn download" download>Download Template</a>-->
                    </div>
                    <!--  </div> -->
                </form>
                <form id="file_up_download" method="POST" action="{{URL::to('download_templetepmupload')}}">
                    {{ csrf_field()}}
...
$(document).ready(function() {
    ...
    $("#editdata").click(function() {
        ...
        var cellValue = $("#grid1").jqGrid('getCell', gr, 'pm_hrd_id'); //alert(cellValue);
    ...
    $(".download").click(function() {
        ...
        $.ajax({
            url: "{{URL::to('download_templetepmupload')}}",
            type: "POST",
            data: form_data,
        ...
        if (file_upload != '') {
            ...
            $.ajax({
                ...
                if (data == 1) {
                    ...
                    notyMsgs('success', 'PM Machine Uploaded Successfully');
                    setTimeout(function() {
                        location.reload();
                    }, 2000);
                } else if (data == 2) {
    ...
    $(document).on('click', '.delete', function(e) {
        ...
        var cellValue = jQuery("#grid1").jqGrid('getCell', gr, 'pm_hrd_id');
        var batch_status = jQuery("#grid1").jqGrid('getCell', gr, 'batch_status');
        if (cellValue) {
            if (batch_status == "LOADED") {
                ...
                $.post("{{url('checklistuploaddelete')}}", {
                    id: cellValue
                ...
                // $.get('checklistuploaddelete/'+cellValue, function(data,status)
                // {
                //     if(data == 0)
                //     {
                //             notyMsg("success","Deleted successfully");
