...
<form id="create-user" action="{{URL::to('companyform')}}" method="POST" style="display: none;">
                                    {{ csrf_field()}}
    ...
    <form id="view-user" action="{{URL::to('companyview')}}" method="POST" style="display: none;">
                                    {{ csrf_field()}}
                                <input type='hidden' name='id' value='0' id='id' class='id'>   
...
/*end*/ 
/* deepika purpose: code for edit button****/ 
 $('#edit').on('click',function(){
        var gr = jQuery("#grid1").jqGrid('getGridParam','selrow');
  var id1 = jQuery("#grid1").jqGrid ('getCell', gr, 'company_id');
   var id=$('.id').val(id1);
   if(gr){ 
       var id=$('.id').val(id1);
document.getElementById('create-user').submit(); 
...
$(document).on('click','.del',function(e){
        ...
        var formdata    = $('#delete-user').serialize();
        ...
        if(cellValue)
        {
            swal({
                ...
                cancelButtonText: 'No',
                 ...
                 if(e == true)
                 {
                    $.post('companydelete/'+cellValue,formdata, function(data,status)
                    {
                        var data = $.trim(data);
                        if(data =='1')
                        {
