...
class ReportController extends Controller
{
  ...
  public function monthlybreakdownreportsearch($sd = null, $ed = null)
  {
    ...
    $lwh = '';
    if ($loc != 0) {
      $lwh = ' and location_id=' . $loc;
    }

    $data_break = \DB::select("select * from m_breakdowntype_t where 1=1 $lwh");
    $data_depart = \DB::select("select * from ma_department_t where 1=1 $lwh");
    $select = '';
    $select1 = '';
    $select_fr = '';
    $selectfre = '';
...
main.group_id,main.department_id)v1 where 1=1 and machine!=''";
  ...
  public function breakdownsummarysearch($sd = null, $ed = null)
  {
    ...
    $data = DB::select('SELECT count(id) as call_raised, sum(IF((request_status = "CANCELLED"),1,0)) AS cancelled,
sum(IF((request_status = "CLOSED"), 1,0)) AS resolved,
sum(IF((request_status = "OPEN" or request_status = "REJECTED" or request_status = "INITIATED"),1,0)) AS pending_resolve,
sum(IF((request_status = "REQUESTED"),1,0)) AS pending_ackn,
SEC_TO_TIME(SUM(TIME_TO_SEC(TIMEDIFF(end_date,start_date)))) AS break_down_hr 
FROM `b_maintenance_t` where 1=1 ' . $wh1);
  ...
  public function categorywisereportsearch($sd = null, $ed = null, $breakdowntype_id = null)
  {
    ...
    if ($loc != 0) {
      ...
      $wh1 .= " and b_maintenance_t.location_id = " . $loc;
      $bwh .= " and location_id=" . $loc;
    }
    $bd_type = $breakdowntype_id;
    $category_name = \DB::SELECT("SELECT breakdown_name FROM m_breakdowntype_t WHERE breakdowntype_id=$bd_type $bwh");
  ...
  public function historycardsearch($sd = null, $ed = null, $machine_id = null)
  {
    ...
    if ($machine_id != 0) {
      $category_name = \DB::SELECT("SELECT machine_name,machine_id FROM machine_hdr_t WHERE machine_id=$machine_id ");
      $wh1 .= " AND machine_hdr_t.machine_id=$machine_id";
    } else {
      $category_name = \DB::SELECT("SELECT machine_name,machine_id FROM machine_hdr_t ");
    }
    $return_data['machine_name'] = $category_name;
  ...
  FROM
  b_maintenance_t  JOIN machine_hdr_t ON machine_hdr_t.machine_id=b_maintenance_t.machine_id where 1=1 $wh1  )v1 where 1=1");
    $data = collect($data);
    $return_data['data'] = $data;
    // print_r($data);
