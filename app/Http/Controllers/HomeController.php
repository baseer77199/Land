...
class HomeController extends Controller
{
    ...
    public function index()
    {
            ...
            foreach ($month as $month_no => $month_name) {
                if ($loc != 0) {
                    $machine_details = \DB::select("SELECT *,
                                                ...
                                                AND machine_id='67' and location_id=$loc");
                } else {
                    $machine_details = \DB::select("SELECT *,
                    if(SEC_TO_TIME(TIMESTAMPDIFF(SECOND, issue_date, end_date)) is null,'00:00:00',SEC_TO_TIME(TIMESTAMPDIFF(SECOND, issue_date, end_date)))  as breakdown_hours,
                    if(SEC_TO_TIME(TIMESTAMPDIFF(SECOND, start_date, end_date)) is null,'00:00:00',SEC_TO_TIME(TIMESTAMPDIFF(SECOND, start_date, end_date))) as repair_hours 
        ...
        //Baseer : Purpose  For MTTR & MTTF NEW  CODE START
        $mttr = [];
        $mtbf = [];
        $year = date('Y');
        ...
        n.month_num,
        n.no_of_bd,
        TIME_FORMAT(SEC_TO_TIME(n.total_hrs),'%H:%i:%s') as total_hrs, 
        SEC_TO_TIME(n.bd_time) as bd_time,
        SEC_TO_TIME(n.repair_time) as repair_time,
        TIME_FORMAT(SEC_TO_TIME(GREATEST((n.total_hrs - n.bd_time) / n.no_of_bd, 0)),'%H:%i:%s') as mtbf,
        TIME_FORMAT(SEC_TO_TIME(n.repair_time / n.no_of_bd),'%H:%i:%s') as mttr 
    FROM ( 
        SELECT 
            DATE_FORMAT(mt.issue_date,'%Y-%M') as issue_month,
            DATE_FORMAT(mt.issue_date,'%m') as month,
            MONTH(mt.issue_date) as month_num,
            COUNT(mt.id) as no_of_bd, 
            SUM(TIME_TO_SEC(TIMEDIFF(mt.end_date, mt.issue_date))) as bd_time,
            SUM(TIME_TO_SEC(TIMEDIFF(mt.end_date, mt.start_date))) as repair_time,
            SUM(DAY(LAST_DAY(mt.issue_date)) * (CASE WHEN m.day_working_hrs IS NULL OR m.day_working_hrs = '' OR m.day_working_hrs = 0 THEN 8 ELSE m.day_working_hrs END) * 3600) as total_hrs
        FROM `b_maintenance_t` mt 
        JOIN machine_hdr_t m ON m.machine_id = mt.machine_id 
        WHERE YEAR(mt.issue_date) = ?
            AND MONTH(mt.issue_date) = ?
            AND mt.is_breakdown = 'Yes' 
            AND mt.end_date IS NOT NULL
        GROUP BY YEAR(mt.issue_date), MONTH(mt.issue_date)
    ) n", [$year, $i]);

            if (!empty($data)) {
                // Convert to float if needed
                $mttrValue = $this->convertTimeToDecimal($data[0]->mttr);
                $mtbfValue = $this->convertTimeToDecimal($data[0]->mtbf);

                // Ensure float type
                $mttr[] = is_numeric($mttrValue) ? (float) $mttrValue : 0;
                $mtbf[] = is_numeric($mtbfValue) ? (float) $mtbfValue : 0;
            } else {
                // Use float 0 instead of string '0'
                $mttr[] = 0.0;
                $mtbf[] = 0.0;
            }
        }
        ...
        //Baseer : Purpose  For MTTR & MTTF NEW  CODE END
        ...
        $new_brkdata = \DB::select("select n.month,n.machine_name,n.no_of_bd, TIME_FORMAT(SEC_TO_TIME(n.total_hrs),'%H:%i:%s') as total_hrs, SEC_TO_TIME(n.bd_time) as bd_time,SEC_TO_TIME(n.repair_time) as repair_time,TIME_FORMAT(SEC_TO_TIME(GREATEST((n.total_hrs-n.bd_time)/n.no_of_bd, 0)),'%H:%i:%s') as mtbf,TIME_FORMAT(SEC_TO_TIME(n.repair_time/n.no_of_bd),'%H:%i:%s')  as mttr from (SELECT DATE_FORMAT(mt.issue_date,'%Y-%M') as issue_month,DATE_FORMAT(mt.issue_date,'%m') as month,count(mt.id) as no_of_bd, sum(TIME_TO_SEC(TIMEDIFF ( mt.end_date,mt.issue_date))) as bd_time,sum(TIME_TO_SEC(TIMEDIFF ( mt.end_date,mt.start_date))) as repair_time,m.machine_name,CASE WHEN m.day_working_hrs IS NULL OR m.day_working_hrs = '' OR m.day_working_hrs = 0 THEN 8 ELSE m.day_working_hrs END as day_working_hrs, (day(last_day(mt.issue_date))*(CASE WHEN m.day_working_hrs IS NULL OR m.day_working_hrs = '' OR m.day_working_hrs = 0 THEN 8 ELSE m.day_working_hrs END) * 3600) as total_hrs FROM `b_maintenance_t` mt join machine_hdr_t m on m.machine_id=mt.machine_id where year(mt.issue_date) ='$year' and mt.is_breakdown='Yes' and date( mt.end_date) !=0 $wh GROUP BY mt.machine_id) n");
        //print_r($new_brkdata); 
        $newbrkdata = \DB::select("select n.month,n.issue_month, n.no_of_bd, TIME_FORMAT(SEC_TO_TIME(n.total_hrs),'%H:%i:%s') as total_hrs, SEC_TO_TIME(n.bd_time) as bd_time,SEC_TO_TIME(n.repair_time) as repair_time,TIME_FORMAT(SEC_TO_TIME(GREATEST((n.total_hrs-n.bd_time)/n.no_of_bd, 0)),'%H:%i:%s') as mtbf,TIME_FORMAT(SEC_TO_TIME(n.repair_time/n.no_of_bd),'%H:%i:%s')  as mttr from (SELECT DATE_FORMAT(mt.issue_date,'%Y-%M') as issue_month,DATE_FORMAT(mt.issue_date,'%m') as month,count(mt.id) as no_of_bd, sum(TIME_TO_SEC(TIMEDIFF ( mt.end_date,mt.issue_date))) as bd_time,sum(TIME_TO_SEC(TIMEDIFF ( mt.end_date,mt.start_date))) as repair_time,m.machine_name,CASE WHEN m.day_working_hrs IS NULL OR m.day_working_hrs = '' OR m.day_working_hrs = 0 THEN 8 ELSE m.day_working_hrs END as day_working_hrs, (day(last_day(mt.issue_date))*(CASE WHEN m.day_working_hrs IS NULL OR m.day_working_hrs = '' OR m.day_working_hrs = 0 THEN 8 ELSE m.day_working_hrs END) * 3600) as total_hrs FROM `b_maintenance_t` mt join machine_hdr_t m on m.machine_id=mt.machine_id where year(mt.issue_date) ='$year' and mt.is_breakdown='Yes' and date( mt.end_date) !=0 $wh GROUP BY DATE_FORMAT(mt.issue_date,'%Y-%m')) n");
        // print_r($newbrkdata); die;
        $whb = "and (`b_maintenance_t`.`request_status` = 'OPEN' or `b_maintenance_t`.`request_status` = 'INITIATED' or `b_maintenance_t`.`request_status` = 'REQUESTED')";
        if ($loc == 0) {
            $total_breakdown = \DB::table('b_maintenance_t')->select('b_maintenance_t.machine_id')->get();
            $open_breakdown = \DB::table('b_maintenance_t')->select('b_maintenance_t.machine_id')->where('b_maintenance_t.request_status', '!=', 'CLOSED')->where('b_maintenance_t.request_status', '!=', 'CANCELLED')->get();
            //$closed_breakdown = \DB::table('b_maintenance_t')->select('b_maintenance_t.machine_id')->where('b_maintenance_t.request_status','CLOSED')->orwhere('b_maintenance_t.request_status','CANCELLED')->get();
            $closed_breakdown = \DB::select("select b_maintenance_t.machine_id FROM b_maintenance_t where 1=1 and (request_status='CLOSED' or request_status='CANCELLED') ");


        } else {
            $total_breakdown = \DB::table('b_maintenance_t')->select('b_maintenance_t.machine_id')->where('b_maintenance_t.location_id', $loc)->get();
            $open_breakdown = \DB::table('b_maintenance_t')->select('b_maintenance_t.machine_id')->where('b_maintenance_t.request_status', '!=', 'CLOSED')->where('b_maintenance_t.request_status', '!=', 'CANCELLED')->where('b_maintenance_t.location_id', $loc)->get();
            $closed_breakdown = \DB::select("select b_maintenance_t.machine_id FROM b_maintenance_t where location_id='" . $loc . "' and (request_status='CLOSED' or request_status='CANCELLED') ");
            // $closed_breakdown = \DB::table('b_maintenance_t')->select('b_maintenance_t.machine_id')->where('b_maintenance_t.location_id',$loc)->where('b_maintenance_t.request_status','CLOSED')->orwhere('b_maintenance_t.request_status','CANCELLED')->get();
            $whb .= " and `b_maintenance_t`.`location_id` = '" . $loc . "'";
            ...
            //     ->where('b_maintenance_t.location_id',$loc)->where('b_maintenance_t.request_status','OPEN')->orwhere('b_maintenance_t.request_status','INITIATED')->orwhere('b_maintenance_t.request_status','REQUESTED')
            //     ->get();
// print_r($data);die;
        }

        $SQL = "select `b_maintenance_t`.`issue_date`, `b_maintenance_t`.`ticket_number`, `b_maintenance_t`.`maintenance_type`, `b_maintenance_t`.`causes`, `ma_department_t`.`department_name`, `machine_hdr_t`.`machine_name`, `m_breakdowntype_t`.`breakdown_name`, `breakdown_severity`.`severity_name`, `b_maintenance_t`.`active`, `t_name`.`first_name` as `tech_name`, `e_name`.`first_name` as `eng_name`, `b_maintenance_t`.`request_status` from `b_maintenance_t` left join `ma_department_t` on `ma_department_t`.`department_id` = `b_maintenance_t`.`department_id` left join `machine_hdr_t` on `machine_hdr_t`.`machine_id` = `b_maintenance_t`.`machine_id` left join `m_breakdowntype_t` on `m_breakdowntype_t`.`breakdowntype_id` = `b_maintenance_t`.`break_type_id` left join `breakdown_severity` on `breakdown_severity`.`breakdownseverity_id` = `b_maintenance_t`.`breakdown_sevearity` left join `tb_users` as `e_name` on `e_name`.`id` = `b_maintenance_t`.`engineer` left join `tb_users` as `t_name` on `t_name`.`id` = `b_maintenance_t`.`technician` where 1=1 $whb";
  ...
  LIMIT 5)v1 where 1=1 and v1.machine!='' ");
    ...
    // Function to convert time format HH:MM:SS to H.MM
    public function convertTimeToDecimal($time)
    {
        ...
        // Format as H.MM (minutes should always be 2 digits)
...
//   ->select('machine_hdr_t.machine_name as title','machine_pm_detail_t.actual_pm_date as start')->where('user_clearance_by','like',"'%".'"'.$id.'"'."%'")->where('status','=',0)->get();
    ...
    public function categoryval()
    {
        $loc = \Session::get('location');
        if ($loc != 0) {
            $category = \DB::select("select m_breakdowntype_t.breakdown_name as name,CONVERT(count(b_maintenance_t.id),SIGNED INTEGER) as y,SEC_TO_TIME(SUM(TIMESTAMPDIFF(SECOND,start_date,end_date))) as tot_time from b_maintenance_t left join m_breakdowntype_t on m_breakdowntype_t.breakdowntype_id=b_maintenance_t.break_type_id where 1=1 and b_maintenance_t.location_id=$loc group by m_breakdowntype_t.breakdown_name");
        } else {
            $category = \DB::select("select m_breakdowntype_t.breakdown_name as name,CONVERT(count(b_maintenance_t.id),SIGNED INTEGER) as y,SEC_TO_TIME(SUM(TIMESTAMPDIFF(SECOND,start_date,end_date))) as tot_time from b_maintenance_t left join m_breakdowntype_t on m_breakdowntype_t.breakdowntype_id=b_maintenance_t.break_type_id where 1=1 group by m_breakdowntype_t.breakdown_name");
        }
