...
//        }   
        ...
        $sql = "SELECT 
        machine_pm_detail_t.*,
        DATE_FORMAT(machine_pm_detail_t.actual_pm_date,'%d-%m-%Y') as actual_pm_date,
        tb_users.username,
        machine_hdr_t.machine_name,
        machine_hdr_t.location_id,
        ma_department_t.department_name 
        from machine_pm_detail_t 
        left join tb_users on(tb_users.id=machine_pm_detail_t.created_by) 
        left join machine_hdr_t on(machine_hdr_t.machine_id=machine_pm_detail_t.machine_id)
         left join ma_department_t on(ma_department_t.department_id=machine_pm_detail_t.department_id)
        where 1=1 $wh1 $wh and machine_pm_detail_t.initiate_status=0 
        order by machine_pm_detail_t.actual_pm_date asc 
        ";
...
//echo ("SELECT COUNT(machine_pm_detail_t.initiate_pm_id) AS count FROM machine_pm_detail_t left join tb_users on(tb_users.id=machine_pm_detail_t.created_by) left join machine_hdr_t on(machine_hdr_t.machine_id=machine_pm_detail_t.machine_id) left join ma_department_t on(ma_department_t.department_id=machine_pm_detail_t.department_id) where 1=1 and  machine_pm_detail_t.initiate_pm_id in ($pm_id) and machine_pm_detail_t.status=0 and machine_pm_detail_t.initiate_status=1 $wh");
    ...
    public function pmmonthlycheckData()
    {
        ...
        $sql = "SELECT machine_pm_detail_t.actual_pm_date,machine_pm_detail_t.user_clearance_by,machine_pm_detail_t.initiate_pm_id,machine_pm_detail_t.pm_no,tb_users.username,machine_hdr_t.machine_name,ma_department_t.department_name,0 as pm_checking_id 
        from machine_pm_detail_t 
        left join tb_users on(tb_users.id=machine_pm_detail_t.created_by) 
        left join machine_hdr_t on(machine_hdr_t.machine_id=machine_pm_detail_t.machine_id)
        left join ma_department_t on(ma_department_t.department_id=machine_pm_detail_t.department_id) 
        LEFT JOIN pm_monthly_checking_tbl pmchk  ON pmchk.initate_pm_id = machine_pm_detail_t.initiate_pm_id
        where 1=1 and machine_pm_detail_t.status=2  AND pmchk.pm_checking_id IS NULL   $wh $wh1 ORDER BY $sidx $sord LIMIT $start , $limit";
...
//        }   
    ...
    public function pmmonthlychecksave(Request $request)
    {
        ...
        try {
            ...
            //var_dump(isset($_POST['savestatus']) && $_POST['savestatus'] == 'APPROVED'); die;
            // var_dump($this->model); die;
            $id = $this->model->insertRow($pm_data);

            if (isset($_POST['savestatus']) && $_POST['savestatus'] == 'APPROVED') {
