...
class BreakdownmaintenanceController extends Controller
{
  ...
  public function create($id = null)
  {
    ...
    if (!is_numeric($id) && ($btnval != "approverequest" || $btnval != "requestraise" || $btnval != "allocateengineer" || $btnval != "allocatetechnician" || $btnval != "closerequest" || $btnval != "closedrequest" || $btnval != "sopupload" || $btnval != "createissue")) {
      ...
      if ($id != 0) {
        ...
        $this->data['row']->shift = $this->jCombologin('shift_timing', 'shift_id', 'shift_name', $sql[0]->shift);

        if ($btnval == 'closedrequest') {
          $productdata = \DB::SELECT("select * from b_maintenance_t_lines where id=$id");
          $this->data['productdata'] = $productdata;
          foreach ($productdata as $pk => $pv) {
            $this->data['row']->spares_id = $this->jCombologin('m_spares_t', 'spares_id', 'spares_name', $pv->spares_id);
          }
        }
       ...
       $this->data['row']->end_date="";*/
        ...
        $cur_date = date('H:i:s');
        $cur_shift = \DB::select(" select * from shift_timing where 1=1 and ('$cur_date' between time(start_time) and time(end_time))");
        if (count($cur_shift) > 0) {
          $shift = $cur_shift[0]->shift_id;
        } else {
          $shift = '';
        }
        $this->data['row']->shift = $this->jCombologin('shift_timing', 'shift_id', 'shift_name', $shift);
  ...
  public function save(Request $request)
  {
    ...
    if ($edit_id == "") {
      ...
      // Auto-allocate technicians when ticket is created
      $tech_data = [];
      $eng_hist = [];
      $eng_hist['breakdown_id'] = $edit_id;
      $eng_hist['created_by'] = $tech_data['allocate_engineer_by'] = \Session::get('id');
      $eng_hist['created_at'] = $tech_data['allocate_engineer_on'] = date("Y-m-d H:i:s");
      $tech_data['priority_option'] = $request->input('priority_option') ?? 'Low';
      ...
      // Fetch all technicians from users where department is_technician = 'Yes'
      ...
      if (count($userIds) > 0) {
        ...
        $tech_data['technician'] = json_encode($userIdsStr);
      } else {
        $eng_hist['technician'] = json_encode([]);
        $tech_data['technician'] = json_encode([]);
      }

      // Insert into engineer history table
      \DB::table('engineer_history_tbl')->insert($eng_hist);

      // Update ticket with technician and status
      $tech_data['request_status'] = "INITIATED";
      Breakdownmaintenance::find($edit_id)->update($tech_data);

      // Send technician allocation notification
      $noti_msg_alloc = "Ticket Allocated Successfully - " . $breakdown->ticket_number;
      $noti_msg1_alloc = "Breakdown Ticket Allocated Successfully for the machine " . $data_machine[0]->machine_name . " - [" . $breakdown->ticket_number . "] on " . date('d-m-Y H:i:s', strtotime($tech_data['allocate_engineer_on'])) . " by " . $this->getusername(\Session::get('id'));
      ...
      $not_data1['edit_id'] = $edit_id;
      $not_data1['noti_msg'] = $noti_msg_alloc;
      $not_data1['noti_msg1'] = $noti_msg1_alloc;
      ...
      $not_data1['status'] = 0;
      \DB::table('pending_noti_t')->insert($not_data1);
      ...
      //  $notification['body']=$noti_msg1;
      //  $notification['title']=$noti_msg;
      //  $notification['icon']="";
      //  $notification['sound']="default";
      //  $count['body']=$noti_msg1;
      //  $count['title']=$noti_msg;
      //  $count['count']="28";
      //  $count['sound']="default";
      ...
      // }
      return response()->json(array('status' => 'success', 'message' => 'Breakdown Maintenance Saved Successfully', 'id' => $edit_id, 'no' => $breakdown->ticket_number));
    } else if ($_POST['request_status'] == "CLOSED" || $_POST['request_status'] == 'UPDATED') {
      ...
      $data['break_type_id'] = $_POST['break_type_id'];
      $data['causes'] = $_POST['causes'];
      $data['breakdown_sevearity'] = $_POST['breakdown_sevearity'];
      $data['priority_option'] = $_POST['priority_option'];
      $data['maintenance_type'] = $_POST['maintenance_type'];
      ...
      //print_r($_POST['request_status']);die;
      /*    dd($data);*/
      if ($_POST['request_status'] == 'UPDATED') {
        $data['request_status'] = "CLOSED";

        $b_ins = \DB::insert("INSERT INTO `b_maintenance_history_t`(`id`, `breakdown_id`, `department_id`, `machine_id`, `break_type_id`, `issue_date`, `breakdown_sevearity`, `priority_option`, `causes`, `ticket_number`, `maintenance_type`, `ticket_count`, `company_id`, `location_id`, `organization_id`, `created_by`, `created_at`, `last_updated_by`, `updated_at`, `active`, `engineer`, `technician`, `request_status`, `critical_spare`, `others`, `start_date`, `corrective_action`, `preventive_action`, `end_date`, `issue_created_by`, `issue_created_on`, `issue_updated_by`, `issue_updated_on`, `is_breakdown`, `allocate_engineer_by`, `allocate_engineer_on`, `request_request_by`, `request_request_on`, `request_approve_by`, `request_approve_on`, `closed_engineer_by`, `closed_engineer_on`, `request_remark`, `approve_remarks`, `shift`, `error_code`, `files`,`changed_by`,`changed_at`) SELECT null,`id`, `department_id`, `machine_id`, `break_type_id`, `issue_date`, `breakdown_sevearity`, `priority_option`, `causes`, `ticket_number`, `maintenance_type`, `ticket_count`, `company_id`, `location_id`, `organization_id`, `created_by`, `created_at`, `last_updated_by`, `updated_at`, `active`, `engineer`, `technician`, `request_status`, `critical_spare`, `others`, `start_date`, `corrective_action`, `preventive_action`, `end_date`, `issue_created_by`, `issue_created_on`, `issue_updated_by`, `issue_updated_on`, `is_breakdown`, `allocate_engineer_by`, `allocate_engineer_on`, `request_request_by`, `request_request_on`, `request_approve_by`, `request_approve_on`, `closed_engineer_by`, `closed_engineer_on`, `request_remark`, `approve_remarks`, `shift`, `error_code`, `files`,'" . \Session::get('id') . "','" . date('Y-m-d H:i:s') . "' FROM `b_maintenance_t` WHERE id='$edit_id'");
        $bh_id = \DB::getPdo()->lastInsertId();
        $bl_ins = \DB::insert("INSERT INTO `b_maintenance_history_t_lines`(`bm_lines_id`, `id`, `line_no`, `spares_id`, `qty`, `inventory_stock`, `company_id`, `location_id`, `organization_id`, `created_by`, `created_at`, `last_updated_by`, `updated_at`) SELECT null,'$bh_id' , `line_no`, `spares_id`, `qty`, `inventory_stock`, `company_id`, `location_id`, `organization_id`, `created_by`, `created_at`, `last_updated_by`, `updated_at` FROM `b_maintenance_t_lines` WHERE id='$edit_id'");
      }
      /*end*/
      Breakdownmaintenance::find($edit_id)->update($data);
      ...
      if ($_POST['request_status'] == 'CLOSED') {
        ...
        // $mailto = $this->getNotySettings($data_machine[0]->location_id,$data_machine[0]->department_id,'close_ticket');
        ...
        //   $notification['body']=$noti_msg;
        // $notification['title']=$noti_msg1;
        // $notification['icon']="";
        // $notification['sound']="default";
        // $count['body']=$noti_msg;
        // $count['title']=$noti_msg1;
        // $count['count']="28";
        // $count['sound']="default";
      ...
      if ($_POST['request_status'] == "INITIATED") {
        ...
        $not_data['location'] = \Session::get('location');
        $not_data['dept_id'] = $data_machine[0]->department_id;
        $not_data['module'] = 'allocate_technician';
        $not_data['edit_id'] = $edit_id;
        $not_data['noti_msg'] = $noti_msg;
        $not_data['noti_msg1'] = $noti_msg1;
        $not_data['user'] = \Session::get('id');
      ...
      if ($_POST['request_status'] == "REQUESTED") {
        ...
        $not_data['location'] = \Session::get('location');
        $not_data['dept_id'] = $data_machine[0]->department_id;
        $not_data['module'] = 'closure_request';
        $not_data['edit_id'] = $edit_id;
        $not_data['noti_msg'] = $noti_msg;
        $not_data['noti_msg1'] = $noti_msg1;
        $not_data['user'] = \Session::get('id');
        $not_data['btn_val'] = 'createissue';
        $not_data['created_at'] = date('Y-m-d H:i:s');
        // $popnoti['company_id']=\Session::get('companyid');
        $not_data['location_id'] = \Session::get('location');
        $not_data['organization_id'] = \Session::get('organization');
      ...
      if ($_POST['request_status'] == "APPROVED") {
        ...
        $not_data['location'] = \Session::get('location');
        $not_data['dept_id'] = $data_machine[0]->department_id;
        $not_data['module'] = 'approve_request';
        $not_data['edit_id'] = $edit_id;
        $not_data['noti_msg'] = $noti_msg;
        $not_data['noti_msg1'] = $noti_msg1;
        $not_data['user'] = \Session::get('id');
        $not_data['btn_val'] = 'closerequest';
        $not_data['created_at'] = date('Y-m-d H:i:s');
        // $popnoti['company_id']=\Session::get('companyid');
        $not_data['location_id'] = \Session::get('location');
        $not_data['organization_id'] = \Session::get('organization');
      ...
      if ($_POST['request_status'] == "REJECTED") {
        ...
        $not_data['location'] = \Session::get('location');
        $not_data['dept_id'] = $data_machine[0]->department_id;
        $not_data['module'] = 'reject_request';
        $not_data['edit_id'] = $edit_id;
        $not_data['noti_msg'] = $noti_msg;
        $not_data['noti_msg1'] = $noti_msg1;
        $not_data['user'] = \Session::get('id');
  ...
  public function issueData($type = null)
  {
    ...
    if ($_GET['status'] == "allocatetechnician") {
      $allocate = 1;
      $wh .= " and b_maintenance_t.request_status = 'OPEN'";
    }
    ...
    $result = \DB::select("SELECT COUNT(b_maintenance_t.id) AS count 
...
// dd($this->data);
  ...
  public function issuecancel(Request $request)
  {
    $id = $_POST['id'];
    $update['request_status'] = "CANCELLED";
    $update['cancelled_by'] = \Session::get('id');
    $update['cancelled_on'] = date('Y-m-d H:i:s');
    $breakdown = \DB::table('b_maintenance_t')->where('id', $id)->update($update);
    return 1;
  }
