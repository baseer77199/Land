...
$validatedData=   $this->validate($request, [
		...
		if($edit_id == '')
		{
            ...
            $shifttimings->location_id=\Session::get('location');
            $shifttimings->organization_id=\Session::get('organization');
            $shifttimings->company_id=\Session::get('companyid');
            $shifttimings->created_by=\Session::get('id');
            $shifttimings->last_updated_by=\Session::get('id');
		...
		else
		{
            ...
            $input['last_updated_by'] =  \Session::get('id');
            $input['company_id'] =  \Session::get('companyid');
            $input['location_id'] =  \Session::get('location');
            $input['organization_id'] =  \Session::get('organization');
    ...
    if($loc != 0){
        $wh1='and  shift_timing.location_id='.$loc;
    }

	$result = \DB::select("SELECT COUNT(shift_id) AS count FROM shift_timing where 1=1 $wh $wh1");
