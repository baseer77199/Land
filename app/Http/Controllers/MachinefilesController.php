            ...
            $edit_id=$_POST['machine_file_id'];
             ...
             $machinefiles['department_id']=$_POST['department_id'];
              //  $machinefiles['machine_name']=$_POST['machine_name'];
                ...
                $machinefiles['file_name'] =$_POST['file_name'];
                $machine = \DB::table('m_machine_file_t')->where('machine_file_id',$edit_id)->get();
                
                if($request->hasfile('upload_file'))
                {
                  if(count($machine)>0){
                    $filess = $machine[0]->upload_file;
                    if (file_exists(public_path().'/upload/machinefile/'.$filess)) {
                      unlink(public_path().'/upload/machinefile/'.$filess);
                   }
                  }
