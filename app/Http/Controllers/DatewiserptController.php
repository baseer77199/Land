...
left join m_location_t as loc ON loc.location_id=machine_hdr_t.location_id
where 1=1 $wh1 and date(b_maintenance_t.issue_date) BETWEEN '$start_date' and '$end_date') as v1 where 1=1 $wh  ORDER BY $sidx";
  ...
  {
    return view('datewisereport.histtable', $this->data);
  }

  public function gethistdatewiserpt()
  {
...
m_breakdowntype_t.breakdown_name,
machine_hdr_t.machine_name,
b_maintenance_history_t.start_date,
b_maintenance_history_t.id,
  if(b_maintenance_history_t.end_date='0000-00-00 00:00:00','-',b_maintenance_history_t.end_date) as end_date,
  if(b_maintenance_history_t.issue_date='0000-00-00 00:00:00','-',b_maintenance_history_t.issue_date) as issue_date,
  b_maintenance_history_t.request_status,
TIMEDIFF(b_maintenance_history_t.end_date,b_maintenance_history_t.start_date) as diftime1,
TIMEDIFF(b_maintenance_history_t.end_date,b_maintenance_history_t.issue_date) as actvltime1,
b_maintenance_history_t.ticket_number,
b_maintenance_history_t.changed_at,
concat(cb.employee_number,'-',cb.first_name) as created_by,
concat(cbb.employee_number,'-',cbb.first_name) as closed_by,
concat(chb.employee_number,'-',chb.first_name) as changed_byy
FROM b_maintenance_history_t 
left join  ma_department_t on (ma_department_t.department_id=b_maintenance_history_t.department_id)
left join  m_breakdowntype_t on(m_breakdowntype_t.breakdowntype_id=b_maintenance_history_t.break_type_id) 
left join  machine_hdr_t on(machine_hdr_t.machine_id=b_maintenance_history_t.machine_id)
left join tb_users as cb ON cb.id=b_maintenance_history_t.issue_created_by
left join tb_users as cbb ON cbb.id=b_maintenance_history_t.closed_engineer_by
left join tb_users as chb ON chb.id=b_maintenance_history_t.changed_by
left join m_location_t as loc ON loc.location_id=machine_hdr_t.location_id
where 1=1 $wh1 and date(b_maintenance_history_t.issue_date) BETWEEN '$start_date' and '$end_date') as v1 where 1=1 $wh  ORDER BY $sidx";
    //  dd("df");   
    ...
    $new_id = count($result);
    $result[$new_id] = (object) array();
    $result[$new_id]->changed_byy = "Format No ";
    $result[$new_id]->changed_at = "04.2211.71.03-8";
    $new_id = count($result);
    $result[$new_id] = (object) array();
    $result[$new_id]->changed_byy = "REV ";
    $result[$new_id]->changed_at = "00/16";
