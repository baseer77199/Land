...
//die(\Request::route()->getName());
    ...
    public function __construct()
    {
        ...
        $this->middleware('auth')->except(['passwordhash', 'sendticketmail', 'escalationmail']);
...
// print_r($menu);exit;
                    // print_r($menu);die;
                    // print_r($data);die;
                    // print_r(!isset($data[$menu]));die;
                    // print_r(array_search(\Request::route()->action['menu'],\Session::get('menu_data'))=='');die;
                    if (\Session::get('groupid') == 1) {
                        ...
                        if (!empty($check)) {
                            ...
                            if (!isset($data[$menu]) && array_search(\Request::route()->action['menu'], \Session::get('menu_data')) == '') {
                                ...
                                if (\Request::route()->getName() != 'downloadtemplate') {
                                    return redirect('permissioindenied');
                                }
                            }
    ...
    */
    ...
    public function jCombologin($table, $option, $display, $selects)
    {
        ...
        if ($loc != 0) {
            if ($table == 'tb_users') {
                $cond .= ' and  loc_id=' . $loc;
            } else if ($table == 'user_department_t' || $table == 'm_company_t' || $table == 'm_cities_t' || $table == 'm_states_t' || $table == 'm_countries_t' || $table == 'shift_timing' || $table == 'frequency_t') {
    ...
    public function jCombologintechnician($table, $option, $display, $selects, $cond_col)
    {
        ...
        if ($loc != 0) {
            if ($table == 'tb_users') {
                $cond .= ' and  loc_id=' . $loc;
            } else if ($table == 'user_department_t' || $table == 'm_company_t' || $table == 'm_cities_t' || $table == 'm_states_t' || $table == 'm_countries_t' || $table == 'shift_timing') {
...
//    else
        ...
        print_r("select DISTINCT CONCAT(" . $dis_name . ") as name ," . $option . " from " . $table . " where locationid=" . $location);
        die;
        $html = "<option value=''>-- Please Select --</option>";
    ...
    public function escalationmail()
    {
        ...
        foreach ($escalation_hdr as $k => $v) {
            ...
            foreach ($escalation_lines as $lk => $lv) {
                ...
                $level = $lv->level;
                $type = $lv->type;
                $escalation_line_id = $lv->escalation_lines_id;
...
// if($bk==1){$bv->department_id=2;}
                    ...
                    $tomailids = array('isac@i5tech.in', 'vigneshs@ifive.in');
                    $ccmailids = array('cs.vigneshvj@gmail.com');
                    $escalation_history = \DB::table('escalation_history_t')->where('breakdown_id', $breakdown_id)->where('escalation_line_id', $escalation_line_id)->get();
                    if (count($escalation_history) <= 0) {
                        // print_r($tomailids);echo "\r\n";print_r($ccmailids);die;
                        $histdata['breakdown_id'] = $breakdown_id;
                        $histdata['escalation_line_id'] = $escalation_line_id;
                        $histdata['noti_msg'] = $data['msg'];
                        $histdata['to_mail_ids'] = implode(',', $tomailids);
                        $histdata['cc_mail_ids'] = implode(',', $ccmailids);
                        $histdata['created_at'] = date('Y-m-d H:i:s');

                        \DB::table('escalation_history_t')->insert($histdata);
    ...
    public function escalationmail_old()
    {
        $escalation_hdr = \DB::select("select * from m_escalation_hrd_tbl where active='Yes'");
        // $escalation_data = \DB::select("select * from m_escalation_lines_tbl join m_escalation_hrd_tbl on m_escalation_hrd_tbl.escalation_hrd_id = m_escalation_lines_tbl.escalation_hrd_id where active='Yes'");
        foreach ($escalation_hdr as $k => $v) {
...
// if($bk==1){$bv->department_id=2;}
        ...
        if ($id != '') {
            ...
            if (count($breakdown) > 0) {
                ...
                if ($module == 'ticket_allocated') {
                    ...
                    $history .= "Ticket Closure Request Raised by " . $data['request_by'] . " (" . date('d-m-Y H:i:s', strtotime($data['request_on'])) . ")" . "\r\n" . "<br>";
                    $history .= "Ticket Closure Request Approved by " . $data['closed_by'] . " (" . date('d-m-Y H:i:s', strtotime($data['closed_on'])) . ")" . "\r\n" . "<br>";
                }
            }
        }
        return $history;
    ...
    public function notimessageUpdate(Request $request)
    {
        $notif_id = $request->input('notif_id');
        $btnval = $request->input('btnval');
        ...
        try {
            ...
            return response()->json(['status' => 'success']); // Optional response
        } catch (\Exception $e) {
    ...
    {
        $pending_mails = \DB::table('pending_noti_t')->where('status', 0)->get();
        foreach ($pending_mails as $pk => $pv) {
            $mailto = $this->getNotySettings($pv->location, $pv->dept_id, $pv->module);
            //print_r($mailto);die;
            if ($mailto['to'] != '') {
                $mto = $mailto['to'];
            } else {
            ...
            $noti_msg = $pv->noti_msg;
            $noti_msg1 = $pv->noti_msg1;

            if ($pv->module == 'ticket_generate') {
                $mm = "ticket_generated";
                $page = "allocatetechnician";
            } else if ($pv->module == 'close_ticket') {
                $mm = "ticket_closed";
                $page = "";
            } else if ($pv->module == 'allocate_technician') {
                $mm = "ticket_allocated";
                $page = "requestraise";
            } else if ($pv->module == 'closure_request') {
                $mm = "ticket_requested";
                $page = "approverequest";
            } else if ($pv->module == 'approve_request') {
                $mm = "ticket_approved";
                $page = "closerequest";
            } else if ($pv->module == 'reject_request') {
                $mm = "ticket_rejected";
                $page = "requestraise";
            }
            ...
            if ($mto != '') {
                $this->sendPopUpHomeNoty($pv->edit_id, "Ticket Generate", $noti_msg, $noti_msg1, $mm, $page, $mto, $mcc, $pv->user, $pv->btn_val);
                ...
                foreach ($fcm_data as $key => $val) {
                    $msg1[] = $this->fcmMsgNotificationcont($val->fcm_token, $notification, $count);
                }
            }
            $up['sent_to'] = $mto;
            $up['updated_at'] = date('Y-m-d H:i:s');
            $up['status'] = 1;
            \DB::table('pending_noti_t')->where('id', $pv->id)->update($up);
        }
        return 'completed';
    }

    public function fcmMsgNotificationcont($token, $res, $count)
    {
    ...
    // function safeInput($attribute, $value, $check, $fail) {
    ...
    //     }
    //     if(in_array('nourlnoemail',$check)) {
    //         // Check if the value is a valid email address
    //         if (preg_match("/^$regex$/i", $value)) // `i` flag for case-insensitive
    //         {
    //             return $fail($attribute . ' is invalid (contains url).');
    //         }
    //     }
    //     if(in_array('noequal',$check)) {
    //         // Check if the value contains the equal sign (=)
    //         if (strpos($value, '=') !== false) {
    //             return $fail("The $attribute contains the equal sign (=), which is not allowed.");
    //         }
    //     }
    //     if(in_array('noscript',$check)) {
    //         // Check for HTML or scripting elements
    //         if (preg_match('/<[^>]*>|script|onerror|onload|iframe|object|embed/i', $value)) {
    //             return $fail("The $attribute contains HTML or scripting elements.");
    //         }
    //     }
    //     if(in_array('nohtml',$check)) {
    //         // Check for HTML or scripting elements
    //         if (preg_match('/<[^>]*>|script|onerror|onload|iframe|object|embed/i', $value)) {
    //             return $fail("The $attribute contains HTML or scripting elements.");
    //         }
    ...
    //     if(in_array('nourl',$check)) {
    //         // If the value contains a URL pattern, return an error
    //         if (preg_match("/(https?|ftp):\/\/[^\s]+/i", $value)) {
    //             return $fail("The $attribute contains a URL, which is not allowed.");
    //         }
    //     }
    //     if(in_array('noscript',$check)) {
    //         // Check if the value contains "script" (case-sensitive)
    //         if (str_contains($value, 'script')) {
    //             return $fail($attribute . ' contains script.');
    //         }
    //     }
    //     if(in_array('noequal',$check)) {
    //         // Check if the value contains equal sign "=" (case-sensitive)
    ...
    function safeInput($attribute, $value, $check, $fail)
    {
        ...
        foreach ($values as $singleValue) {
            ...
            if (in_array('nourl', $check)) {
                if (preg_match("/^$regex$/i", $singleValue)) {
            ...
            if (in_array('noequal', $check)) {
                if (strpos($singleValue, '=') !== false) {
                    return $fail("The $attribute contains the equal sign (=), which is not allowed.");
                }
            }

            if (in_array('noscript', $check)) {
                if (preg_match('/<[^>]*>|script|onerror|onload|iframe|object|embed/i', $singleValue)) {
                    return $fail("The $attribute contains HTML or scripting elements.");
                }
                if (str_contains($singleValue, 'script')) {
                    return $fail($attribute . ' contains script.');
                }
            }
            ...
            if (in_array('nourl', $check)) {
                if (preg_match("/(https?|ftp):\/\/[^\s]+/i", $singleValue)) {
                    return $fail("The $attribute contains a URL, which is not allowed.");
                }
            }
        }
    ...
    {
        $userId = auth()->id(); // or \Session::get('id')
        $companyId = \Session::get('companyid');
        $notificationId = $request->input('notification_id');

        try {
            $notification = \DB::table('notifications_t')
            ...
            \DB::table('notifications_t')
                ->where('notification_id', $notificationId)
                ->update(['read/unread' => 'read', 'updated_at' => now()]);
    ...
    public function getNotificationCount(Request $request)
    {
        ...
        $count = \DB::table('notifications_t')
            ->where('read/unread', 'unread')
            ->where('user_id', $userId)
            ->where('company_id', $companyId)
            ->count();
