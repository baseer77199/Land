 ...
 left join m_amc_tb as amc on(amc.vendor_id=machine_hdr_t.amc_vendor_id) where 1=1 $wh1)v1 where 1=1 $wh ORDER BY v1.machine_id";
  ...
  public function create($id = null)
  {
    ...
    if (!is_numeric($id)) {
      ...
      if ($id != '0') {
        ...
        $this->data['row']->relocated_date = ($this->data['row']->relocated_date == '1970-01-01' || $this->data['row']->relocated_date == '0000-00-00') ? '' : $this->data['row']->relocated_date;
        $this->data['row']->purchased_date = ($this->data['row']->purchased_date == '1970-01-01' || $this->data['row']->purchased_date == '0000-00-00') ? '' : $this->data['row']->relocated_date;
        $this->data['row']->renewal_date = ($this->data['row']->renewal_date == '1970-01-01' || $this->data['row']->renewal_date == '0000-00-00') ? '' : $this->data['row']->renewal_date;
        $this->data['row']->from_date = ($this->data['row']->from_date == '1970-01-01' || $this->data['row']->from_date == '0000-00-00') ? '' : $this->data['row']->from_date;
        $this->data['row']->to_date = ($this->data['row']->to_date == '1970-01-01' || $this->data['row']->to_date == '0000-00-00') ? '' : $this->data['row']->to_date;
      ...
      /*deepika purpose:to create new bom*/ else {
        // $loc=\Session::get('location');
        ...
        $this->data['row']->relocated_date = '';
        $this->data['row']->purchased_date = '';
        $this->data['row']->renewal_date = '';
  ...
  public function save(Request $request)
  {
    ...
    $validatedData = $this->validate($request, [
      ...
      //'relocated_date' => 'nullable|date_format:d-m-Y|after_or_equal:'.$todayDate,
      //'purchased_date' => 'nullable|date_format:d-m-Y|after_or_equal:'.$todayDate,
      'machine_make' => [
        // function ($attribute, $value, $fail) {
        //   $regex = "((https?|ftp)\:\/\/)?"; // SCHEME 
        //   $regex .= "([a-z0-9+!*(),;?&=\$_.-]+(\:[a-z0-9+!*(),;?&=\$_.-]+)?@)?"; // User and Pass 
      ...
      //'renewal_date' =>  'nullable|date_format:d-m-Y|after_or_equal:'.$todayDate,
      //'from_date' =>  'nullable|date_format:d-m-Y|after_or_equal:'.$todayDate,
      //'to_date' => 'nullable|date_format:d-m-Y|after_or_equal:'.$stop_date,
