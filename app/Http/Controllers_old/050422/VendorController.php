 ...
 //dd($dbCode);
    ...
    public function getvendorData($type=null){
     ...
     $wh='';
        ...
        if($start <0) $start = 0;
        $SQL = "SELECT m_amc_tb.*,tb_users.username,tb_users.id as created_id FROM m_amc_tb left join tb_users on tb_users.id =m_amc_tb.created_by where 1=1  $wh ORDER BY $sidx $sord LIMIT $start , $limit";
        
        $download_SQL = "SELECT m_amc_tb.*,tb_users.username,tb_users.id as created_id FROM m_amc_tb left join tb_users on tb_users.id =m_amc_tb.created_by where 1=1  $wh ORDER BY $sidx $sord";
