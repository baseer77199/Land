 ...
 return view('report.pmdaily', $this->data);       
	     ...
	     public function getpmyearlydata($i_year = null,$dept_id=null) {
			 ...
			 $i_year=$_GET['start_date'];
                ...
                $this->data['machine_details']=$machine_details=\DB::select("SELECT machine_hdr_t.*,machine_lines_t.frequency_date from machine_hdr_t left join machine_lines_t on(machine_lines_t.machine_id=machine_hdr_t.machine_id) where machine_hdr_t.department_id='$dept_id'");
     ...
     $loc=\Session::get('location');
                ...
                $this->data['machine_details']=$machine_details=\DB::select("SELECT machine_hdr_t.*,machine_lines_t.frequency_date from machine_hdr_t left join machine_lines_t on(machine_lines_t.machine_id=machine_hdr_t.machine_id) where machine_hdr_t.department_id='$dept_id' and machine_hdr_t.location_id='$loc'");
...
$sidx='';
 ...
 $department_data=\DB::select("SELECT ma_department_t.*,machine_pm_detail_t.*,machine_hdr_t.*,Case WHEN machine_pm_detail_t.status=0 then 'Not Initiated' WHEN machine_pm_detail_t.status=1 then 'Initiated' WHEN machine_pm_detail_t.status=2 then 'Pending' WHEN machine_pm_detail_t.status=3 then 'Completed' End AS status FROM ma_department_t LEFT join machine_pm_detail_t on machine_pm_detail_t.department_id = ma_department_t.department_id left join machine_hdr_t on machine_hdr_t.machine_id = machine_pm_detail_t.machine_id  where machine_pm_detail_t.actual_pm_date='$start_date' and ma_department_t.department_id ='$department_id' $wh1 ");
...
$count = count($department_data);
    ...
    if($start <0) $start = 0;
       ...
       $sql=\DB::select("SELECT ma_department_t.*,machine_pm_detail_t.*,machine_hdr_t.*,Case WHEN machine_pm_detail_t.status=0 then 'Not Initiated' WHEN machine_pm_detail_t.status=1 then 'Initiated' WHEN machine_pm_detail_t.status=2 then 'Pending' WHEN machine_pm_detail_t.status=3 then 'Completed' End AS status FROM ma_department_t LEFT join machine_pm_detail_t on machine_pm_detail_t.department_id = ma_department_t.department_id left join machine_hdr_t on machine_hdr_t.machine_id = machine_pm_detail_t.machine_id  where machine_pm_detail_t.actual_pm_date='$start_date' and ma_department_t.department_id ='$department_id' $wh $wh1");
  
   $download_SQL="SELECT ma_department_t.*,machine_pm_detail_t.*,machine_hdr_t.*,Case WHEN machine_pm_detail_t.status=0 then 'Not Initiated' WHEN machine_pm_detail_t.status=1 then 'Initiated' WHEN machine_pm_detail_t.status=2 then 'Pending' WHEN machine_pm_detail_t.status=3 then 'Completed' End AS status FROM ma_department_t LEFT join machine_pm_detail_t on machine_pm_detail_t.department_id = ma_department_t.department_id left join machine_hdr_t on machine_hdr_t.machine_id = machine_pm_detail_t.machine_id  where machine_pm_detail_t.actual_pm_date='$start_date' and ma_department_t.department_id ='$department_id' $wh $wh1";
