...
dd($dbCode);
    ...
    {
        $loc=\Session::get("location");
    	//$this->data['asset_code'] = $this->jComboformcomb('machine_hdr_t','machine_id','asset_code',''); 
    	 $this->data['asset_code'] = $this->jCombolocation('machine_hdr_t','machine_id','asset_code','','and location_id=$loc'); 
...
//Upload
        ...
        {		
          ...
          //  $uploadSuccess = $file->move($destinationPath, $filename);
            ...
            if ($ext == "csv") 
            {
                ...
                $id = DB::table('hr_emp_upload_details_new_t')->insertGetId(['batch_no' =>$batch_no]);
             $cantain=0;
				...
				for($inc=1;$inc<=21;$inc++){
					if($filesop[$inc]!='')
					{
						...
						//dd($filesop);		
								...
								$attendencedata['created_by']=\Session::get('emp_id');
        ...
        { 
            ...
            foreach($result_pr as $key=>$value)
            {
               ...
               if(!empty($value->locationid))
                {
                    $checkvalue =$this->checkvalue('m_location_t',$value->locationid,"location_name");
   ...
   if(!empty($value->group_id))
                {
                    $checkvalue =$this->checkvalue('machine_group_t',$value->group_id,"group_name");
                ...
                {
                    $checkvalue =$this->checkvalue('m_amc_tb',$value->vendor_id,"vendor_name");
                ...
                {
                    $status['status'] = 'error';
                    $status['message'].= 'Please enter Vendor' . ' ,';
                  }
                    if(!empty($value->amc_vendor_id))
                {
                    $checkvalue =$this->checkvalue('m_amc_tb',$value->amc_vendor_id,"vendor_name");
                ...
                {
                    $status['status'] = 'error';
                    $status['message'].= 'Please enter AMC Vendor' . ' ,';
                  }
                    if(!empty($value->frequency))
                {
                    $checkvalue =$this->checkvalue('frequency_tbl',$value->frequency,"frequency_name");
    ...
    public function LoadMaster($loadModname, $status){
      ...
      { 
        foreach($result  as $key=>$value)
        {
          ...
          $location =$this->checkvalue('m_location_t',$value->locationid,"location_name");
          $group =$this->checkvalue('machine_group_t',$value->group_id,"group_name");
				  $vendor =$this->checkvalue('m_amc_tb',$value->vendor_id,"vendor_name");
				  $amc_vendor =$this->checkvalue('m_amc_tb',$value->amc_vendor_id,"vendor_name");
          ...
          $hdr_details['locationid'] = $location[0]->location_id;
				  ...
				  $hdr_details['location_id']=$user_details['location_id']=$value->location_id;
				  $hdr_details['organization_id']=$user_details['organization_id']=$value->organization_id;
				
				  $hdr_details['created_by']=$user_details['created_by']=$value->created_by;
				  $hdr_details['created_at']=$user_details['created_at']=$value->created_at;
          ...
          $hdr_details['company_id'] =$user_details['company_id'] = \Session::get('companyid');
          $hdr_id = DB::table('machine_hdr_t')->insertGetId($hdr_details);
				 
          foreach($result1  as $key1=>$value1){
              /***************** Load Employee Officia Details ************************************/
            $frequency =$this->checkvalue('frequency_tbl',$value1->frequency,"frequency_name");
