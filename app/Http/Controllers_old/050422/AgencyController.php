...
return view('Agency.form',$this->data);
    ...
    $result = \DB::select("SELECT COUNT(agency_id) AS count FROM  ma_agency_t where 1=1 $wh");
    ...
    if($start <0) $start = 0;
 $sql="SELECT ma_agency_t.*,tb_users.username from ma_agency_t left join tb_users on(tb_users.id=ma_agency_t.created_by) where 1=1 $wh ORDER BY $sidx $sord  LIMIT $start,$limit";      
     
     $download_SQL="SELECT ma_agency_t.*,tb_users.username from ma_agency_t left join tb_users on(tb_users.id=ma_agency_t.created_by) where 1=1 $wh ORDER BY $sidx $sord";
...
return view('Agency.docform',$this->data);
    ...
    $start = $limit*$page - $limit;
    if($start <0) $start = 0;
 $sql="SELECT document_tb.*,tb_users.username,frequency_tbl.frequency_name,ma_department_t.department_name,machine_hdr_t.machine_name from document_tb left join tb_users on(tb_users.id=document_tb.created_by) left join frequency_tbl on(frequency_tbl.frequency_id=document_tb.frequency) left join ma_department_t on(ma_department_t.department_id=document_tb.department_id) left join machine_hdr_t on machine_hdr_t.machine_id=document_tb.machine_id where 1=1  and status=0 and machine_hdr_t.locationid=$locat_id $wh ORDER BY $sidx $sord  LIMIT $start,$limit";      
 $download_SQL="SELECT document_tb.*,tb_users.username,frequency_tbl.frequency_name,ma_department_t.department_name,machine_hdr_t.machine_name from document_tb left join tb_users on(tb_users.id=document_tb.created_by) left join frequency_tbl on(frequency_tbl.frequency_id=document_tb.frequency) left join ma_department_t on(ma_department_t.department_id=document_tb.department_id) left join machine_hdr_t on machine_hdr_t.machine_id=document_tb.machine_id where 1=1  and status=0 and machine_hdr_t.locationid=$locat_id $wh ORDER BY $sidx $sord";
 $result1 = \DB::select( $download_SQL );
        $result1=collect($result1)->map(function($x){ return (array) $x; })->toArray();
    if(isset($_GET['download']))
    {
 ...
 //   dd($result);
  ...
  public function getdocumentData(agency $agency)   
    ...
    $result = \DB::select("SELECT document_tb.*,tb_users.username,frequency_tbl.frequency_name,ma_department_t.department_name,machine_hdr_t.machine_name from document_tb left join tb_users on(tb_users.id=document_tb.created_by) left join frequency_tbl on(frequency_tbl.frequency_id=document_tb.frequency) left join ma_department_t on(ma_department_t.department_id=document_tb.department_id) left join machine_hdr_t on machine_hdr_t.machine_id=document_tb.machine_id where 1=1 and machine_hdr_t.locationid=$locat_id $wh");
    $count = count($result);
    if( $count > 0 && $limit > 0)
        {
            $total_pages = ceil($count/$limit);
    ...
    $start = $limit*$page - $limit;
    if($start <0) $start = 0;
 $sql="SELECT document_tb.*,tb_users.username,frequency_tbl.frequency_name,ma_department_t.department_name,machine_hdr_t.machine_name from document_tb left join tb_users on(tb_users.id=document_tb.created_by) left join frequency_tbl on(frequency_tbl.frequency_id=document_tb.frequency) left join ma_department_t on(ma_department_t.department_id=document_tb.department_id) left join machine_hdr_t on machine_hdr_t.machine_id=document_tb.machine_id where 1=1 and machine_hdr_t.locationid=$locat_id $wh ORDER BY $sidx $sord  LIMIT $start,$limit";      
     $download_SQL="SELECT document_tb.*,tb_users.username,frequency_tbl.frequency_name,ma_department_t.department_name,machine_hdr_t.machine_name from document_tb left join tb_users on(tb_users.id=document_tb.created_by) left join frequency_tbl on(frequency_tbl.frequency_id=document_tb.frequency) left join ma_department_t on(ma_department_t.department_id=document_tb.department_id) left join machine_hdr_t on machine_hdr_t.machine_id=document_tb.machine_id where 1=1 and machine_hdr_t.locationid=$locat_id $wh ORDER BY $sidx $sord";
 $result1 = \DB::select( $download_SQL );
        $result1=collect($result1)->map(function($x){ return (array) $x; })->toArray();
    if(isset($_GET['download']))
    {
