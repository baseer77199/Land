...
class BreakdowntypeController extends Controller
{
    ...
    public function getGridData($type=null)
    {
       // dd("dhg");
        ...
        if(!$sidx) $sidx =1;
        $result = \DB::select("SELECT COUNT(breakdowntype_id) AS count FROM m_breakdowntype_t left join tb_users on (tb_users.id =m_breakdowntype_t.created_by) where 1=1 $wh");
        ...
        $SQL = "SELECT m_breakdowntype_t.*,tb_users.username,tb_users.id as created_id FROM m_breakdowntype_t left join tb_users on tb_users.id =m_breakdowntype_t.created_by where 1=1 and m_breakdowntype_t.company_id=$compy $wh ORDER BY $sidx $sord LIMIT $start , $limit";
       // dd($SQL);
        $download_SQL = "SELECT m_breakdowntype_t.*,tb_users.username,tb_users.id as created_id FROM m_breakdowntype_t left join tb_users on tb_users.id =m_breakdowntype_t.created_by where 1=1 and m_breakdowntype_t.company_id=$compy $wh ORDER BY $sidx $sord";
