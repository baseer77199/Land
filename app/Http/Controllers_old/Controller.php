...
public function getnoty(){
 ...
 return $data;
     ...
     public function sendPopUpHomeNoty($poID,$module_name,$noti_message,$pageurl,$id,$cid,$btnval){
        ...
        if(count($notification_setting) > 0){
            ...
            foreach($users as $k=>$v){
                ...
                if(count($users)>0){
                    $email = $users[0]->email;
                    $daata['updated_at'] = $users[0]->first_name;
                    $subject = "Notification Mail";
                    if($email!=null && $email!=''){
                        \Mail::send('sendnotimail',$daata, function($message) use($email,$subject)
                        {
                            $to=$email;
                            $message->to($to);
...
public function jCombologin($table,$option,$display,$selects)
{
    ...
    if($loc_id!=0){
        if($table=='tb_users'){
            $cond = " and loc_id=".$loc_id;    
        }else if($table=='user_department_t'){
            
        }else{
            $cond = " and location_id=".$loc_id;
        }
    ...
    if($table == "m_countries_t" )
        $select=\DB::select("select DISTINCT CONCAT(".$dis_name.") as name ,".$option." from ".$table ." where country_id != '6' ".$cond );
    else if( $table =="m_states_t")
        $select=\DB::select("select DISTINCT CONCAT(".$dis_name.") as name ,".$option." from ".$table ." where state_id != '41' ".$cond);
    else if($table =="m_cities_t" )
        $select=\DB::select("select DISTINCT CONCAT(".$dis_name.") as name ,".$option." from ".$table ." where city_id!='1' ".$cond);
    else
