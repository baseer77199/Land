     ...
     public function sendPopUpHomeNoty($poID,$module_name,$noti_message,$pageurl,$id,$cid,$btnval){
        ...
        if(count($notification_setting) > 0){
            ...
            foreach($users as $k=>$v){
                ...
                $popnoti['company_id']=1;
                $popnoti['btn_val']=$btnval;
                $popnoti['created_by']=$cid;
                ...
                if(count($users)>0){
                    $email = $users[0]->email;
                    $popnoti['emp_name'] = $users[0]->first_name;
                    $subject = "Notification Mail";
                    if($email!=null && $email!=''){
                        \Mail::send('sendnotimail',$popnoti, function($message) use($email,$subject)
                        {
                            $to=$email;
                            $message->to($to);
