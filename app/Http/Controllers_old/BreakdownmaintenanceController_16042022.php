...
class BreakdownmaintenanceController extends Controller
{
      ...
      public function create($id=null)
    {
     ...
     // dd($id);
     // print_r($_POST);   print_r($btnval);die;
    ...
    if($sql[0]->end_date=='0000-00-00 00:00:00'){
     ...
     // print_r($this->data['row']);die;
        ...
        $technician=json_decode($sql[0]->technician);
                ...
                $this->data['row']->technician=$this->jcustommultiselect('tb_users','id','first_name',$technician,'');
        }else{
          //dd('hh');
        $this->data['row']= (object)array();
       ...
       $validatedData=   $this->validate($request, [
        ...
        // dd($breakdown);
            ...
            $id_notiy=rtrim($id_notiy, ',');
             $noti_msg = "Ticket Generated ". $breakdown->ticket_number." - ".$data_machine[0]->machine_name;
...
// echo 'hi';die;
      ...
      /*   dd($_POST);*/
          ...
          'engineer' => 'integer|numeric|required',
          'technician' => [
                function($attribute, $value, $fail) {
        ...
        // dd('hi');
            ...
            $this->sendPopUpHomeNoty($edit_id,"Ticket Generate",$noti_msg,"engineerallocate",$_POST['engineer'],\Session::get('id'),'requestraise');
            ...
            $this->sendPopUpHomeNoty($edit_id,"Ticket Generate",$noti_msg,"engineerallocate",$id_notiy,\Session::get('id'),'approverequest');
            }
            ...
            $this->sendPopUpHomeNoty($edit_id,"Ticket Generate",$noti_msg,"engineerallocate",$issue_user[0]->issue_created_by,\Session::get('id'),'closerequest');
             ...
             $data['request_approve_on'] = date("Y-m-d");
              $message="Rejected Successfully";
               $noti_msg = "Rejected Successfully ". $_POST['ticket_number']." - ".$data_machine[0]->machine_name;
                 $issue_user=\DB::SELECT("select issue_created_by from b_maintenance_t where id=$edit_id");
            $this->sendPopUpHomeNoty($edit_id,"Ticket Generate",$noti_msg,"userview",$issue_user[0]->issue_created_by,\Session::get('id'),'createissue');
    ...
    $wh=$this->jqgridsearch('b_maintenance_t',$_GET['filters'],$search_table);
     ...
     if($_GET['status']=="createissue"){
      ...
      if($_GET['status']=="allocatetechnician"){
         $wh.=" and (b_maintenance_t.request_status='INITIATED' or b_maintenance_t.request_status='REJECTED' or b_maintenance_t.request_status='OPEN')";
    }
     if($_GET['status']=="requestraise"){
         $wh.=" and (b_maintenance_t.request_status='INITIATED' or b_maintenance_t.request_status='REJECTED') and engineer=$id";
     }
     if($_GET['status']=="approverequest"){
         $wh.=" and (b_maintenance_t.request_status='REQUESTED' or b_maintenance_t.request_status='REJECTED')";
     }
       if($_GET['status']=="closerequest"){
         $wh.=" and b_maintenance_t.request_status='APPROVED' and engineer=$id";
     }
...
if($dept_id==4){
    ...
    $SQL = "SELECT b_maintenance_t.*,ma_department_t.department_name,m_breakdowntype_t.breakdown_name,breakdown_severity.severity_name
