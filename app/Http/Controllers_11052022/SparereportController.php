...
class SparereportController extends Controller
{
        ...
        if (!$sidx)
            $sidx = 1;
              $result = \DB::select("select * from (SELECT
               ...
               from m_spares_t 
               where 1=1  group by m_spares_t.spares_id ) as sp1 where 1=1 $wh $wh1 ");
     //  dd($result);
         $count = count($result); 
		//dd($count);
		if( $count > 0 && $limit > 0)
 ...
 $SQL = "select * from (SELECT
               ...
               m_spares_t.spare_quantity
               from m_spares_t 
               where 1=1  group by m_spares_t.spares_id ) as sp1 where 1=1 $wh $wh1  ORDER BY $sidx  LIMIT $start , $limit";

 $download_SQL="select * from (SELECT
               m_spares_t.spares_name,
               m_spares_t.spare_quantity
               from m_spares_t 
               where 1=1  group by m_spares_t.spares_id ) as sp1 where 1=1 $wh $wh1 ORDER BY $sidx  ";
